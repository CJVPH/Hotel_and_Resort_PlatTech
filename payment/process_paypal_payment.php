...
if (empty($paymentReference)) {
    $back = $isPavilion ? "paypal_payment.php?booking_id=$bookingId" : "paypal_payment.php?reservation_id=$reservationId";
    header('Location: ' . $back . '&error=PayPal transaction ID is required'); exit();
}
...
try {
    ...
    if ($isPavilion) {
        ...
        $stmt->close();
        $upd = $conn->prepare("UPDATE pavilion_bookings SET payment_method='paypal', payment_reference=?, payment_status='pending', status='pending' WHERE id=?");
        ...
        $stmt->close();
        $upd = $conn->prepare("UPDATE reservations SET payment_method='paypal', payment_reference=?, payment_status='pending', status='pending' WHERE id=?");
