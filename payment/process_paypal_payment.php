...
if (empty($paymentReference)) {
    header('Location: paypal_payment.php?reservation_id=' . $reservationId . '&error=PayPal transaction ID is required');
    exit();
}
...
try {
    ...
    if (isset($_FILES['proof_of_payment']) && $_FILES['proof_of_payment']['error'] === UPLOAD_ERR_OK) {
        ...
        $fileName = 'paypal_' . $reservationId . '_' . time() . '.' . $fileExtension;
        ...
        header('Location: paypal_payment.php?reservation_id=' . $reservationId . '&error=Receipt screenshot is required');
        exit();
    }
    ...
    header('Location: paypal_payment.php?reservation_id=' . $reservationId . '&error=Payment submission failed, please try again');
    exit();
}
?>
